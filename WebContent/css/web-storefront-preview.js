...
var n=its.detect.currentPageIsMacAppStore()||its.url.queryParamValue("mt",t)=="12"||t.indexOf("associateVPPUserWithITSAccount")!==-1||t.indexOf("vpp-associate")!==-1,r=its.detect.currentPageIsMacBookStore(),i=!1,s;
...
t.length+f.length<400&&(dbg("openItunes: Attaching "+document.referrer),t+=u+"ign-msr="+f)}}}else!its.detect.itunesDetected()||t.indexOf("associateVPPUserWithITSAccount")===-1&&t.indexOf("vpp-associate")===-1?t=itms.PageData.itunesDownloadUrl:t=t.replace(/^http/,"itms");
...
jQuery("head").append(t),window.addEventListener("load",function(){setTimeout(function(){window.scrollTo(0,1)},0)})}},checkRetina=function(){window.devicePixelRatio&&window.devicePixelRatio>1&&jQuery("body").addClass("hd-device")};
jQuery(document).ready(checkRetina);
